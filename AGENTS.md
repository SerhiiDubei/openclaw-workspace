...
**ПРИНЦИП: Записувати КОЖНЕ повідомлення одразу після відповіді!**

### Перед відповіддю:
1. **Ідентифікувати користувача** — визначити username з Telegram ID
2. **Перевірити файл сесії** — `memory/users/{username}/sessions/YYYY-MM-DD.md`
3. **Якщо мовчання > 30 хв** — створити нову сесію з тим же днем

### Після відповіді (ОБОВ'ЯЗКОВО!) — АВТОМАТИЧНИЙ ЗАПИС:
...
**Параметри:**
- `{username}` — ім'я користувача з Telegram (наприклад: bomberman047, mental-ninja)
- `{user_message}` — повний текст повідомлення користувача
- `{assistant_message}` — повний текст твоєї відповіді
- `{user_id}` — Telegram ID користувача (опціонально)

**Приклад:**
```bash
/root/.openclaw/workspace/scripts/log-current-session.sh "bomberman047" "Привіт" "Привіт, Сергію!" "488426634"
```
